...
This folder contains the source code for the 04_GPIO_Out C++ program 
which is designed to toggle Pin 0 on the microbits card edge connector.

Since we are going to be toggling the state of a pin on card edge connector
it is always interesting to see how fast we can do it. The MainLoop() code
contains 4 methods which you can comment in and out in order to see how 
fast it can go. The results are quite interesting (spoiler alert: 2.31 MHz 
is possible).

You will need to view the output waveform with an oscilloscope. If you do 
not have an oscilloscope it is highly recommended that you read the comments
in the MainLoop() anyways as you will need to know these techniques.

Also, the center LED of the BBC micro:bit array blinks at 1Hz but that is 
not the point of this example. It is just there to provide a visual indication
that the program is running.

Other specific things demonstrated in this example code which you might 
wish to look out for:

  1) The execution time cost of function calls - especially those with 
     parameters.     
  2) The execution time cost of reading a register.  
  3) The injection of "Bare metal" code inline in the C++ code in order to
...
Things you need to know: 
  ...
  9) When ready, run the CompileProgram.bat script. It should complete without
     ...
     to the top of the 04_GPIO_Out directory and running the 
 ...
 10) The successful run of the CompileProgram.bat script will have left a 
     ...
     the microbit. It should automatically load and the center LED 
     should blink at a rate of once per second. If you place an oscilloscope
     probe on Pin 0 of the card edge connector you should see a waveform 
     being output.
