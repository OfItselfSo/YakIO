...
This folder contains the source code for the 06_deBounce C++ program 
which is designed to demonstrate how to debounce the buttons and also 
count button  presses. The count is made visible by sequentially lighting 
LEDs on the 5 x 5 array. Once the LEDs in the array are all on, they are 
all turned off and the process begins again.

Other specific things demonstrated in this example code which you might 
wish to look out for:

  1) Note how the switch debouncing code has been implemented in the Heartbeat
     in order to simplify the code in the MainLoop()      
  2) Note that ButtonA is debounced and ButtonB is not. This may introduce
     changes in their behaviour.
  3) Both ButtonA and ButtonB only trigger on a pressed to unpressed 
     transition and the flag indicating a button pressed event latches
     until reset by the code that handles it.
...
Things you need to know: 
  ...
  9) When ready, run the CompileProgram.bat script. It should complete without
     ...
     to the top of the 06_deBounce directory and running the 
 ...
 10) The successful run of the CompileProgram.bat script will have left a 
     ...
     the microbit. It should automatically load and if either ButtonA or
     ButtonB are pressed the LEDS in the array should light up sequentially.
     Once all LEDs are lit they will all turn off and the process will
     begin again.
