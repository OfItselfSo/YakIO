...
This folder contains the source code for the 01_Blinky C++ program which 
is designed to blink the center LED of the BBC micro:bit array. More to 
the point, this code is intended to illustrate the basic usage of the 
YakIO library and also to provide a simple "get started" example of how
to cross compile C++ code for the ARM Cortex-M0+ Nordic CPUs which 
drive the BBC micro:bit. 

Other specific things demonstrated in this example code which you might 
wish to look out for:

  1) The instantiation of a user written C++ class so that all executing
     code takes place within C++ files.
  2) The creation and use of a "mainloop" function which serves as the 
     central processing loop of an embedded C++ program
  3) The instantiation on the stack of a class from the YakIO library
  4) How to drive and refresh the 5 x 5 LED array on the microbit
  5) How to toggle the state of a LED in the 5 x 5 array
  6) How to use a delay function and counter in the mainloop to achieve
     variable rates of operations
...
Things you need to know: 
  ...
  9) When ready, run the CompileProgram.bat script. It should complete without
     ...
     to the top of the 01_Blinky directory and running the CompileProgram.bat
     script.
 ...
 10) The successful run of the CompileProgram.bat script will have left a 
     ...
     the microbit. It should automatically load and run and you should see 
     the center LED blinking at a rate of once per second. 
